...
10 "Update System" on
11 "Install Basic Xfce Apps and Multimedia Programs" on
...
;;
15)
yes | sudo pacman -S pamac-aur yay update-grub downgrade
;;
16)
...
;;
19)
tar xf xfce-config.tar.gz -C $HOME
;;
20)
...
;;
31)
sudo pacman -S cups system-config-printer --needed --noconfirm
...
;;
3)
sudo pacman -S cdemu-daemon --needed --noconfirm
sed -i 's/mountisoaction=true/mountisoaction=false/g' $HOME/.config/kservicemenurc
;;
4)
sudo pacman -S cdemu-daemon vhba-module-dkms --needed --noconfirm
sed -i 's/mountisoaction=true/mountisoaction=false/g' $HOME/.config/kservicemenurc
;;
5)
